...
bestNum = 5
bestVal =0
# print(pipeline("annotationstest/raw_list.txt", "data/SimpsonsNewTest/", numCentroids = 20, batchSize = 50))
for numCentroids in (math.pow(2, x) for x in range(1, 13)):
	numCentroids = int(numCentroids)
	t0 = time.time()
	val = pipeline("annotationstest/raw_list.txt", "data/SimpsonsNewTest/", numCentroids = numCentroids, batchSize = 50)
