...
def pipeline(labelPath, fileDir, numCentroids = 50, C=1.0, batchSize = 100, num_iter = 1000, subset=False):
	...
	t0 = time.time()
	train_target, test_target, train_data, test_data = train_test_split(labels, data, train_size=0.05, random_state=526198)
...
#	print( "took " + str(time.time() - t0) + " seconds to run")
	...
	print(clf.fit(MFCCs, MFCCLabels))
	print( "took " + str(time.time() - t0) + " seconds to run")
	t0 = time.time()
	outcome = voteOnLabels(clf, test_data, test_target)
	print( "took " + str(time.time() - t0) + " seconds to run")
	f  = open('/home/ma2510/machine_learning/logSVM.txt', 'a')
	writeString = str(C) + "," + str(outcome) +"\n"
	print(writeString)
	f.write(writeString)
	f.close()
...
# bestBatch = 10
# bestVal =0
pipeline("/home/ma2510/machine_learning/annotationstrain/raw_list.txt", "/home/ma2510/machine_learning/data/SimpsonsNewBetterTrain/", C=1.0, numCentroids = 256, batchSize = 500, subset = False)
